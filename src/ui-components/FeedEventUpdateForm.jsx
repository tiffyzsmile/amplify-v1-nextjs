...
import { getFeedEvent } from "../graphql/queries";
import { updateFeedEvent } from "../graphql/mutations";
const client = generateClient();
export default function FeedEventUpdateForm(props) {
  const {
    id: idProp,
    feedEvent: feedEventModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = feedEventRecord
      ? { ...initialValues, ...feedEventRecord }
      : initialValues;
    setUserId(cleanValues.userId);
    setSourceNodeId(cleanValues.sourceNodeId);
  ...
  const [feedEventRecord, setFeedEventRecord] =
    React.useState(feedEventModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getFeedEvent.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getFeedEvent
        : feedEventModelProp;
      setFeedEventRecord(record);
    };
    queryData();
  }, [idProp, feedEventModelProp]);
  React.useEffect(resetStateValues, [feedEventRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateFeedEvent.replaceAll("__typename", ""),
            variables: {
              input: {
                id: feedEventRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "FeedEventUpdateForm")}
      {...rest}
    >
      <TextField
        label="User id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || feedEventModelProp)}
